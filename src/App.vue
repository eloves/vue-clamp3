<template>
  <article id="app">
    ...
    <ul>
      ...
      <li>
        {{ zh ? '支持在布局变化时自动更新。' : 'Automatically updates upon layout change.' }}
      </li>
      <li>
...
&lt;/script&gt;
    ...
    <section>
      <ul>
        <li>
          ...
          <p>
            {{ zh ? '生成的根元素的标签名。' : 'The tag name of the generated root element.' }}
          </p>
          <p>
            {{ defaultText }}
            <code>div</code>
        ...
        <li>
          ...
          <p>
            {{ zh ? '可以显示的最大行数' : 'The max number of lines that can be displayed.' }}
          </p>
        </li>
        <li>
          <p>
        ...
        <li>
          ...
          <p>
            {{ zh ? '截断后显式省略符号的位置。' : 'The location of the ellipsis.' }}
          </p>
          <p>
            {{ defaultText }}
            <code>'end'</code>
        ...
        <li>
          ...
          <p>
            {{ zh ? '是否展开显式被截断的文本。' : 'Whether the clamped area is expanded.' }}
          </p>
          <p>
            {{ defaultText }}
            <code>false</code>
    ...
    <footer>
      <p v-if="zh">
        ...
        和
        <a href="https://github.com/Justineo/vue-clamp/graphs/contributors">贡献者们</a>
        共同创作。
      </p>
      <p v-else>
        Made by
